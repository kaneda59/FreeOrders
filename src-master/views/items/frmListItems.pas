...
{$R *.dfm}
  uses Module, Logs, Consts_, frmInputItem, cli.Datas, classebase;
...
begin
  inherited;
  case id of
     100 : if TformInputItems.Execute(0) then UpdateData;
     200 : if TformInputItems.Execute(qryList.FieldByName('id').AsInteger) then UpdateData;
     300 : if messageDLG('voulez-vous supprimer cet article ?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
             deleteObject(qryList.FieldByName('id').AsInteger, TItems);
  end;
end;
