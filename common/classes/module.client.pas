...
function TModule_Client.Delete: Boolean;
begin
  result:= False;
  qry.Close;
  qry.SQL.Clear;
  qry.SQL.Add('delete from CLIENTS WHERE id=' + IntToStr(fid));
  try
    qry.ExecSQL;
    result:= True;
  except
...
begin
  ...
  begin
    ...
    begin
      qry.SQL.Add('(Id, firstname, lastname, address, supplement, zipcode, city, country, phone, mobile, account, mail)');
      qry.SQL.Add('VALUES');
      qry.SQL.Add('(:Id, :firstname, :lastname, :address, :supplement, :zipcode, :city, :country, :phone, :mobile, :account, :mail)');
    end;
  end
