...
function TModule_orders.Delete: Boolean;
begin
  result:= False;
  qry.Close;
  qry.SQL.Clear;  // suppression des lignes
  qry.SQL.Add('delete from orderLines WHERE orderid=' + IntToStr(fid));
  try
    qry.ExecSQL;

    qry.close;
    qry.SQL.Clear;   // suppression de l'order
    qry.SQL.Add('delete from orders WHERE id=' + IntToStr(fid));
    qry.ExecSQL;
...
begin
  ...
  begin
    ...
    begin
      qry.SQL.Add('(Id, code, idclient, date, stateorder)');
      qry.SQL.Add('VALUES');
      qry.SQL.Add('(:Id, :code, :idclient, :date, :stateorder)');
    end;
  end
...
//      fOrders._id:= Donnees.getLastId('orders');
    ...
    qry.SQL.Add('(Id, orderid, iditems, qte, MtRem)');
    qry.SQL.Add('VALUES');
    qry.SQL.Add('(:Id, :orderid, :iditems, :qte, :MtRem)');

    for i := Low(fOrders._Lines) to High(fOrders._Lines) do
    begin
      qry.Parameters.ParamByName('id').Value       := fORders._Lines[i]._id;
